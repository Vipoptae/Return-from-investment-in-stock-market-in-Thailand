...
# RSI.overbought = RSI$rsi > ovb
# RSI.oversold   = RSI$rsi < ovs
# RSI.up         = RSI$rsi >= lag(RSI$rsi)
# RSI.down       = RSI$rsi <  lag(RSI$rsi)
RSI.signbuy    = RSI$rsi > rsicheck
RSI.signsell   = RSI$rsi < rsicheck
# RSI$sign_RSI = rep(0,nrow(RSI))
# RSI$sign_RSI[diff(RSI.overbought) == 1] = -1
# RSI$sign_RSI[diff(RSI.oversold) == 1] = 1

RSI$sign_RSI[diff(RSI.signbuy) == 1  ] = 1 #diff -> prevent signal overlap
RSI$sign_RSI[diff(RSI.signsell) == 1 ] = -1
# RSI$sign_RSI[diff(RSI.up) == 1] = 1
# RSI$sign_RSI[diff(RSI.down) == 1] = -1
...
# fast.over.slow = STO$fastD > STO$slowD
...
all_buy = merge(RSI$sign_RSI,ADX$sign_ADX)
all_buy[is.na(all_buy)] = 0
all_buy = cbind(all_buy,rowSums(all_buy))
# sigbuy=ifelse(all_buy[,3]&lag(all_buy[,3]),0,all_buy[,3])
